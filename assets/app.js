...
(function () {
  ...
  /* ════════════════════════════════════════
     종가 맞추기: Firebase 연동 + 범위제한 + 어제 랭킹
     ════════════════════════════════════════ */
  function initPredict() {
    const btn = $("#predict-btn");
    const input = $("#predict-input");
    const today = DATA ? DATA.current.date : "";
    const nasdaqPrice = DATA ? DATA.nasdaq.price : null;

    // 현재가 표시 + placeholder
    if (nasdaqPrice) {
      input.placeholder = `현재 ${Math.round(nasdaqPrice).toLocaleString()} (±5%)`;
    }

    // 이미 제출했으면 비활성
    const saved = localStorage.getItem("predict_" + today);
    if (saved) {
      btn.disabled = true;
    ...
    btn.addEventListener("click", async () => {
      ...
      // 범위 체크: 현재가 ±5%
      if (nasdaqPrice) {
        const lo = nasdaqPrice * 0.95;
        const hi = nasdaqPrice * 1.05;
        if (val < lo || val > hi) {
          input.value = "";
          input.placeholder = `${Math.round(lo)}~${Math.round(hi)} 범위만`;
          setTimeout(() => { input.placeholder = `현재 ${Math.round(nasdaqPrice).toLocaleString()} (±5%)`; }, 2000);
          return;
        }
      }
      ...
      if (db) {
        // 중복 체크
        const existing = await db.ref(`predictions/${today}/${ipHash}`).once("value");
        if (existing.exists()) {
          btn.disabled = true;
          btn.textContent = "이미 제출";
          input.value = existing.val().prediction;
          input.disabled = true;
          return;
        }
        await db.ref(`predictions/${today}/${ipHash}`).set({
          prediction: val,
          nick: nick,
          ts: Date.now(),
        });
      ...
      localStorage.setItem("predict_" + today, String(val));
      btn.disabled = true;
      btn.textContent = "제출 완료";
      input.disabled = true;
    });
    ...
    // 어제 랭킹 렌더
    renderYesterdayRanking(today);
  }

  async function renderYesterdayRanking(today) {
    const prevDate = yesterday(today);
    const rankEl = $("#predict-rank");
    const db = getFirebaseDb();

    // 어제 나스닥 종가 찾기
    const prevDay = DATA.history.find((h) => h.date === prevDate);
    const actualClose = prevDay ? prevDay.nasdaq_close : null;
    ...
    if (db) {
      const snap = await db.ref(`predictions/${prevDate}`).once("value");
      const data = snap.val();
      if (data) {
        entries = Object.values(data).map((v) => ({
          nick: v.nick || "익명",
          prediction: v.prediction,
          diff: Math.abs(v.prediction - actualClose),
        }));
      }
    }
    ...
    // 로컬 폴백
    if (!entries.length) {
      const localPred = localStorage.getItem("predict_" + prevDate);
      if (localPred) {
        entries.push({
          nick: getChatNick(),
          prediction: parseFloat(localPred),
          diff: Math.abs(parseFloat(localPred) - actualClose),
        });
      }
    }
    ...
    let html = `<div style="font-size:12px;font-weight:700;margin-bottom:6px;color:#6e6e8a;">어제 랭킹 (${prevDate.slice(5)}) · 종가 ${actualClose.toLocaleString()}</div>`;
    top.forEach((e, i) => {
      const medal = i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `${i + 1}.`;
      const diffStr = e.diff < 1 ? "적중!" : `${e.diff.toFixed(0)}차이`;
      const diffCls = e.diff < 50 ? "up" : "dim";
      html += `<div class="rank-item">
        <span class="rank-pos">${medal}</span>
        <span>${escapeHtml(e.nick)}</span>
        <span class="mono">${Math.round(e.prediction).toLocaleString()}</span>
        <span class="rank-diff ${diffCls}">${diffStr}</span>
      </div>`;
    });
